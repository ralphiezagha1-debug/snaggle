...
export const sendgridTestV2 = onRequest({ region: "us-central1", secrets: [SENDGRID_API_KEY, MAIL_FROM, MAIL_ADMIN] }, async (req, res) => {
    allow(req, res, async () => {
        try {
            const to = (req.method === "GET" ? req.query.to : req.body?.to)?.trim();
            if (!isValidEmail(to ?? ""))
