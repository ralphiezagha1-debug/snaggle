...
const AuctionRoom = () => {
  ...
  // Watch auction + recent bids
  useEffect(() => {
    if (!id) return;

    let auctionUnsub: (() => void) | undefined;
    let bidsUnsub: (() => void) | undefined;

    (async () => {
      const bidApi = await loadBidApi();

      auctionUnsub = bidApi.watchAuction(id, (nextAuction) => {
        setAuction(nextAuction);
      });

      bidsUnsub = bidApi.watchRecentBids(id, 20, (nextBids) => {
        setBids(nextBids ?? []);
      });
    })();

    return () => {
      if (typeof auctionUnsub === "function") auctionUnsub();
      if (typeof bidsUnsub === "function") bidsUnsub();
    };
  }, [id]);
  ...
  const handleBid = async () => {
    if (!currentUser) {
      toast({
        title: "Not signed in",
        description: "You must be signed in to bid.",
        variant: "destructive",
      });
      return;
    }

    if ((userCredits?.balance ?? 0) < 1) {
      toast({
        title: "Insufficient Credits",
        description:
          "You don't have enough bid credits. Purchase more to continue bidding.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      setUserCredits((prev) =>
        prev ? { ...prev, balance: prev.balance - 1 } : null
      );
    } catch (error: any) {
      toast({
        title: "Bid Failed",
  ...
  const handleTimeUp = () => {
    ...
    if (auction?.lastBidderId === currentUser?.uid) {
      toast({
        ...
        description: `You won the ${auction.title} for $${auction.currentPrice.toFixed(
          2
        )}!`,
      });
    } else if (auction) {
      toast({
        title: "Auction Ended",
        description: `${auction.lastBidder || "Someone"} won this auction.`,
      });
    }
  };
              ...
              <CardContent className="p-6">
                <div className="space-y-4">
                  <div className="relative">
                    ...
                    />
                    <Badge
                      className={cn("absolute top-4 right-4 text-lg px-3 py-1", {
          ...
          <div className="space-y-6">
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">
                    {(bids ?? []).filter((b) => b.userId === currentUser?.uid)
                      .length}
                  </span>
                </div>
            ...
            <div className="flex items-center gap-2 p-3 bg-card border rounded-lg">
              ...
              <span className="text-sm">
                {auction.participantCount || 0} people watching
              </span>
            </div>
          </div>
        </div>
